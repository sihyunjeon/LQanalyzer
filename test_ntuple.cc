...
void HNSSSFMuMuE_ntuple::ExecuteEvents()throw( LQError ){
  ...
  // ========== Trigger cut ====================
  bool mumue_trig = false;
  float mumue_trig_weight=1.;
  if(PassTrigger("HLT_DiMu9_Ele9_CaloIdL_TrackIdL_v")) mumue_trig = true;
  mumue_trig_weight = WeightByTrigger("HLT_DiMu9_Ele9_CaloIdL_TrackIdL_v", TargetLumi);

  bool mumu_trig_1 = false;
  float mumu_trig_1_weight=1.;
  if(PassTrigger("HLT_Mu17_Mu8_DZ_v")
  || PassTrigger("HLT_Mu17_Mu8_SameSign_DZ_v")
  || PassTrigger("HLT_Mu20_Mu10_SameSign_DZ_v")) mumu_trig_1 = true;
  mumu_trig_1_weight = WeightByTrigger("HLT_Mu20_Mu10_SameSign_DZ_v", TargetLumi);

  std::vector<TString> dimu_trig;
  bool mumu_trig_2 = false;
  float mumu_trig_2_weight=1.;
  dimu_trig.push_back("HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_v");
  dimu_trig.push_back("HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_v");
  dimu_trig.push_back("HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ_v");
  dimu_trig.push_back("HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_v");
  if( PassTriggerOR(dimu_trig) ) mumu_trig_2 = true;
  mumu_trig_2_weight = WeightByTrigger(dimu_trig, TargetLumi);
  
  bool mue_trig = false;
  float mue_trig_weight=1.;
  if(PassTrigger("HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_v")
  || PassTrigger("HLT_Mu23_TrkIsoVVL_Ele8_CaloIdL_TrackIdL_IsoVL_v")) mue_trig = true;
  mue_trig_weight = WeightByTrigger("HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_v", TargetLumi);

  bool ee_trig = false;
  float ee_trig_weight=1.;
  if(PassTrigger("HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL_DZ_v")
  || PassTrigger("HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL_v")) ee_trig = true;
  ee_trig_weight = WeightByTrigger("HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL_v",TargetLumi);

  bool pass_trig = (mumue_trig || mumu_trig_1 || mumu_trig_2 || mue_trig || ee_trig);
  if(!pass_trig) return;
  // ================================================================================
...
/*    else if(it_sys==13){
    ...
    bool is_2Lmu_1Lel = false;
    bool is_2Tmu_1Tel = false;
    bool is_3Lmu = false;
    bool is_3Tmu = false;
    bool is_3Lel = false;
    bool is_3Tel = false;
    ...
    bool is_SSSF = false;
    bool pass_min_pt_cuts = false;

    bool signal_region = false;
    bool control_region_mumue = false;
    bool control_region_mumumu = false;
    bool control_region_eee = false;
    ...
    if( ((muonLooseColl.size() == 2) && (electronLooseColl.size() == 1)) ){
      is_2Lmu_1Lel = true;
      if( ((muonLooseColl.at(0).Charge()) == (muonLooseColl.at(1).Charge())) && ((muonLooseColl.at(0).Charge() != (electronLooseColl.at(0).Charge())) ){
        is_SSSF = true;
      }
      if( ((muonTightColl.size() == 2) && (electronTightColl.size() == 1)) ){
        is_2Tmu_1Tel = true;
      }
      if( (muonLooseColl.at(0).Pt() > 10) && (muonLooseColl.at(1).Pt() > 5) && (electronLooseColl.at(0).Pt() > 5) ){
        pass_min_pt_cuts = true;
      }
    }

    if( ((electronLooseColl.size() == 3)) )

    if( mumu_trig_2 && is_2Lmu_1Lel && is_2Tmu_1Tel && is_SSSF && pass_min_pt_cuts ) signal_region;
    if( mumu_trig_2 && is_2Lmu_1Lel &&              &&!is_SSSF && pass_min_pt_cuts ) control_region_mumue;
    if( mue_trig    && is_2Lmu_1Lel && 		    &&!is_SSSF && pass_min_pt_cuts ) control_region_e
