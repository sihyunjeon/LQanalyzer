...
void HNSSSFMuMuE::ExecuteEvents()throw( LQError ){
  ...
  // ========== Trigger reweight ====================
  float weight_trigger = 1.;//WeightByTrigger("HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ_v", TargetLumi);//FIXME
  // ================================================================================
  ...
  if(muonTightColl.size() != 1) return;
std::vector<TString> triggerlist;
  triggerslist.push_back("HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_v");
  triggerslist.push_back("HLT_Mu8_TrkIsoVVL_Ele17_CaloIdL_TrackIdL_IsoVL_v");
  triggerslist.push_back("HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_v");
  triggerslist.push_back("HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_v");
  triggerslist.push_back("HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_DZ_v");
  triggerslist.push_back("HLT_Mu17_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_v");
  triggerslist.push_back("HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_DZ_v");
  triggerslist.push_back("HLT_Mu23_TrkIsoVVL_Ele8_CaloIdL_TrackIdL_IsoVL_v");
  triggerslist.push_back("HLT_Mu12_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_v");
  triggerslist.push_back("HLT_Mu12_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_DZ_v");
  triggerslist.push_back("HLT_Mu30_Ele30_CaloIdL_GsfTrkIdVL_v");

  bool triggerpass[5] = {false,};
  if(PassTrigger(triggerslist.at(0)) || PassTrigger(triggerslist.at(1)) || PassTrigger(triggerslist.at(2))) triggerpass[0] = true;
  if(PassTrigger(triggerslist.at(3)) || PassTrigger(triggerslist.at(4))) triggerpass[1] = true;
  if(PassTrigger(triggerslist.at(5)) || PassTrigger(triggerslist.at(6))) triggerpass[2] = true;
  if(PassTrigger(triggerslist.at(7)) || PassTrigger(triggerslist.at(8)) || PassTrigger(triggerslist.at(9))) triggerpass[3] = true;
  if(PassTrigger(triggerslist.at(10))) triggerpass[4] = true;

  for(int i=0; i<5; i++){
    if(triggerpass[i]) FillHist("Signal_TriggerStudy", i, 1, 0., 5., 5);
  }

  snu::KParticle mu, el;
  mu = muonTightColl.at(0);
  el = electronTightColl.at(0);

  if(mu.Charge() != el.Charge()) return;
  FillHist("Signal_NEvents", 0, 1, 0., 1., 1);
...
int HNSSSFMuMuE::GetPeriodIndex(void){
  if( isData ){
