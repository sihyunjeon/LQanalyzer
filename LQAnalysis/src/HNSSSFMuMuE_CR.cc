...
void HNSSSFMuMuE_CR::ExecuteEvents()throw( LQError ){
  ...
  TString mumue_trigger="HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ_v";
  ...
  if( (muonLooseColl.at(0).Pt() < 20) || (muonLooseColl.at(1).Pt() < 10) ) return;
...
/*
void HNSSSFMuMuE_CR::ClearHeader(void){

  RAWmu[0].SetPxPyPzE(0,0,0,0); RAWmu[1].SetPxPyPzE(0,0,0,0); RAWmu[2].SetPxPyPzE(0,0,0,0); RAWel.SetPxPyPzE(0,0,0,0); RAWnu[0].SetPxPyPzE(0,0,0,0); RAWnu[1].SetPxPyPzE(0,0,0,0);
  RECOnu.SetPxPyPzE(0,0,0,0);
  ...
  if( control_region == 1 ){
	...
	if( RAWel.Pt() > 20 ){
	  ...
	  if( RECOnu.Pt() > 30 ){
	    ...
	    if( (RAWmu[0]+RAWmu[1]).M() > 4 && (RAWmu[0]+RAWel).M() > 4 && (RAWmu[1]+RAWel).M() > 4){
	      if( (RAWmu[0]+RAWmu[1]+RAWel).M() > 100 ){
		...
		if( N_bjet == 0 ){
		  ...
		  if( jetColl.size() == 0 ){
		    StudyControlRegion( "0Jets", RAWmu, RAWel, jetColl, RECOnu, control_region, weight ) ;}
		  else if( jetColl.size() == 1 ){
                    StudyControlRegion( "1Jets", RAWmu, RAWel, jetColl, RECOnu, control_region, weight ) ;} 
		  else if( jetColl.size() == 2 ){
                    StudyControlRegion( "2Jets", RAWmu, RAWel, jetColl, RECOnu, control_region, weight ) ;}
                  else{
                    StudyControlRegion( "3orMoreJets", RAWmu, RAWel, jetColl, RECOnu, control_region, weight ) ;}
		}
	      }
	    }
	  }
